...
**Last updated:** 2026-08-09 (P11d job stop).
...
| **P11** | Real terminals and job control | P11a–d ✅ session/pgrp + PTY + n_tty + **job stop** |
...
| Real userspace | Dynamically linked glibc/musl, PTYs, net, graphics/input | 🟡 P10a–d + P11a–d in; P12–P13 still open |
...
**P11d (job stop) landed.** Smoke: `jobstoptest`. P11 practical slice is complete. Next: **P12** BSD sockets (`socket`/`bind`/`connect`) so userspace can use the NIC.
