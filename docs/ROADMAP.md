...
**Last updated:** 2026-08-09 (P11a session/pgrp + console termios).
...
| **P11** | Real terminals and job control | P11a ✅ session/pgrp + console termios/`TIOCSCTTY`; still need PTY pair (`ptmx`/`pts`) + line discipline |
...
| Real userspace | Dynamically linked glibc/musl, PTYs, net, graphics/input | 🟡 P10a–d + P11a in; PTY pair / P12–P13 still open |
...
**P11a (session/pgrp + console termios) landed.** Smoke: `jobtest`. Next: P11b PTY pair (`/dev/ptmx`, pts slave) so a terminal emulator can spawn a shell.
