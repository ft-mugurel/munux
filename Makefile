...
# Freestanding i386 ET_EXEC for the ELF loader (`run /bin/hello`)
userland: ${USERLAND_SRC} ${USERLAND_LD}
	$(call require_tool,$(NASM),nasm)
	$(call require_tool,$(LD),ld)
	@mkdir -p build/userland build/rootfs/bin
	@${NASM} -f elf32 ${USERLAND_SRC} -o build/userland/hello.o
	@${LD} -m elf_i386 -T ${USERLAND_LD} -o ${USERLAND_HELLO} build/userland/hello.o
	@echo -e "$(BOLD)$(GREEN)[✓] USERLAND ${USERLAND_HELLO}$(RESET)"

disk: userland
	@mkdir -p build/rootfs/docs build/rootfs/bin
	@echo 'Hello from KFS ext2!' > build/rootfs/hello.txt
